...
namespace Mkey {
    ...
    public class BubblesShooter : MonoBehaviour
    {
        ...
        #region ShootBubble
        public static ShootBubble shootBubble { get; private set; }
        public static ShootBubble hiddenShootBubble { get; private set; }
        [SerializeField]
        ...
        private List<MainObject> shootBubblesOnBoartAtStart;        // shoot bubbles sequece created at startup
        private List<MainObject> shootBubblesOnBoartAtCurrTime;     // shoot bubbles sequece created at curr time
        ...
        public void InitStart(int minBubbleScore)
        {
            this.minBubbleScore = minBubbleScore;
            int count = MPlayer.LcSet.levelMission.MovesConstrain;
            GoodShoots = 0;
            ...
            // create swapPath
            swapPath = new PolyLine(ProcCurve.ArcCurve2D(bubblePosition.position, nextBubblePosition.position, 3, -swapArcHeight));
            swapPathReverse = swapPath.Reverse();
            initPath = new PolyLine(ProcCurve.ArcCurve2D(instantiateBubblePosition.position, nextBubblePosition.position, 3, swapArcHeight));
            ...
            ShootEvent += BoosterFunc.ShootEventHandler;
            SetNextBubble(null);
        }
        ...
        #region instantiate new shootbubble, destroy
        private ShootBubble InstantiateShootBubble(int id, Vector3 position)
        {
            return InstantiateShootBubble(GoSet.GetMainObject(id),  position);
        }

        private ShootBubble InstantiateShootBubble(GridObject prefab, Vector3 position)
        {
            if(!prefab)
            {
                if(debug) Debug.Log("ShootBubble prefab == null");
                return null;
            }
            ShootBubble sB = Instantiate(prefab, transform).GetOrAddComponent<ShootBubble>();
#if UNITY_EDITOR
            sB.name = "ShootBubble: " + prefab.name;
#endif
            sB.SetData(SortingOrder.MainToFront, true, ShootBubbleClickEventHandler);
            sB.transform.localScale = Vector3.one;
            sB.transform.position = position;
            return sB;
        }

        private ShootBubble InstantiateShootBubbleBooster(BoosterFunc boosterPrefab, Vector3 position )
        {
            if (!boosterPrefab)
            {
            ...
            ShootBubble sB = Instantiate(boosterPrefab, transform).GetOrAddComponent<ShootBubble>();
#if UNITY_EDITOR
            sB.name = "ShootBubble booster: " + boosterPrefab.Name;
#endif
            sB.SetData(SortingOrder.MainToFront, true, ShootBubbleClickEventHandler);
            sB.transform.localScale = Vector3.one;
            sB.transform.position = position;
        ...
        private void SetNextBubble(Action completeCallBack)
        {
            ...
            // select sequence 
            List<MainObject> currShootBubblesSequence = shootBubblesOnBoartAtStart; // default
            ...
            if (currShootBubblesSequence == null || currShootBubblesSequence.Count == 0)
            {
            ...
            if (!shootBubbleNext) // instantiate and move shootBubbleNext to nextBubblePosition
            {
                sbSeq.Add((callBack) =>
                {
                    shootBubbleNext = InstantiateShootBubble(currShootBubblesSequence.GetRandomPos().ID, instantiateBubblePosition.position);
            ...
            if (!shootBubble)
            {
                ...
                {
                    shootBubbleNext = InstantiateShootBubble(currShootBubblesSequence.GetRandomPos().ID, instantiateBubblePosition.position);
                    MoveAlongPath(shootBubbleNext.gameObject, initPath, 0.5f * initPath.Length / swapSpeed, 0, EaseAnim.EaseLinear, callBack);
                });
            }
        ...
        private void GetShootBubblesFromBoard()
        {
            ...
            shootBubblesOnBoartAtCurrTime = new List<MainObject>();
            foreach (var item in moDict)
            {
                MainObject mO = MPlayer.GoSet.GetMainObject(item.Key);
                if (mO.CanUseAsShootBubbles)
                {
                    shootBubblesOnBoartAtCurrTime.Add(mO);
                }
            }
        }

        public void DestroyHiddenShootBubble()
        {
            ShootBubble old = hiddenShootBubble;
            if (old)
            {
                Destroy(old);
            }
        ...
        public void Shoot(Action completeCallBack)
        {
            ...
            SimpleTween.Value(gameObject, 0, rLength, time). // ??? sbgo
                SetOnUpdate((float d) =>
                {
                ...
                AddCompleteCallBack(() =>
                {
                    sB.ApplyShootBubbleToGrid(hitTargetGridCell, freeTargetGridCell, shootGridCellsArea); // destroy and cancel tween
                    SetNextBubble(null);
        ...
        private void HideTargetSelectors()
        {
            ...
            if (shootGridCellsArea != null) { shootGridCellsArea.ShowTargetSelector(false); }
        }
        ...
        private void SetShootTargets(GridCell newHitGCell, Vector2 hitPoint, ShootBubble sB)
        {
            ...
            {
                NeighBors nCells = new NeighBors(hitTargetGridCell, 0);
        ...
        #region handlers
        private void BoosterActivateEventHandler(BoosterFunc b)
        {
            ...
            shootBubble = InstantiateShootBubbleBooster(b, bubblePosition.position); // set new shoot bubble
        }

        private void BoosterDeActivateEventHandler()
        {
        ...
        {
            BoosterFunc.ActiveBooster?.DeActivateBooster();
            Swap(null);
        }
